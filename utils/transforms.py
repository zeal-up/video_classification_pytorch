...
class GroupNormalize(object):
    '''Normalize a list of tensor image with mean and standard deviation.
    input:tensor image of size (C, H, W), value in [0.0, 1.0]'''
...
class GroupRandomScale(object):
    '''Rescales the input PIL.Image to a random size, sampled from a interval [low, high]
    'size' will be the size of the smaller edge.
    For example, if height > width, then image will be
    rescaled to (size * height / width, size)
    size: size of the smaller edge
    ...
    def __init__(self, size_low, size_high, interpolation=Image.BILINEAR):
        self.size_low = size_low
        self.size_high = size_high
        self.interpolation = interpolation

    def __call__(self, img_group):
        size = random.choice(range(self.size_low, self.size_high+1, 1))
        worker = torchvision.transforms.Resize(size, interpolation=self.interpolation)
...
class StackTensor(object):
    '''Stack a list of img tensor of size (C x H x W) to a pytorch tensor of size (C x T x H x W)'''
    ...
    def __call__(self, img_group):
        ...
        img_group = torch.stack(img_group, 1).contiguous()
        return img_group
